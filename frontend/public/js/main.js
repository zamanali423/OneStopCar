(function ($) {
  ...
  const initProductGallery = function (element, layout) {
    ...
    const image = gallery.find(".product-gallery__featured");
    const carousel = gallery.find(".product-gallery__thumbnails");
  ...
  $(function () {
    ...
    $(".block-products-carousel").each(function () {
      ...
      block.find(".section-header__arrow--prev").on("click", function () {
        owlCarousel.trigger("prev.owl.carousel", [500]);
      });
      block.find(".section-header__arrow--next").on("click", function () {
        owlCarousel.trigger("next.owl.carousel", [500]);
      });

      let cancelPreviousGroupChange = function () {};

      block.find(".section-header__groups-button").on("click", function () {
        const carousel = block.find(".block-products-carousel__carousel");

        if ($(this).hasClass("section-header__groups-button--active")) {
          return;
        }
        ...
        let timer = setTimeout(function () {
          ...
          itemsHTML.sort(() => Math.random() - 0.5); // Shuffle

          const newItems = itemsHTML.map(
            (html) =>
              `<div class="block-products-carousel__column">${html}</div>`
          );

          owlCarousel
            .trigger("replace.owl.carousel", [$(newItems.join(""))])
